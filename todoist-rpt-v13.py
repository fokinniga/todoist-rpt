...
# --- FUNCIONES DE OBTENCIÓN DE DATOS (No Modificadas, Asumen que ya funcionan) ---
...
def getProyectList():
    ...
    try:
        ...
        for i, row in df_root_pys.iterrows():
            opciones[i + 1] = row['project_id']
            print(f"{i + 1}. {row['project_name']} (ID: {row['project_id']})")
        ...
        # --- FILTRADO DE PROYECTOS HIJOS ---
        
        # 5. Filtrar el DataFrame completo (df_projects)
        # Se obtienen los proyectos donde su parent_id es el ID seleccionado
        df_proyectos_hijos = df_projects[df_projects['parent_id'] == seleccion_id].copy()
        
        print("\n--- 📂 Proyectos Hijos del Root Seleccionado ---")
        print(df_proyectos_hijos)
        
        # 6. Devolver el DataFrame Filtrado (el que contiene los proyectos 'reporte')
        return df_proyectos_hijos 
